...
# 1. 栈
栈就是我们平时写代码接到的lua_State。在实现上是用一个数组实现的。每个成员类型是TValue，看下文详细介绍。这里不打算详细介绍栈的结构和内容，只会介绍和gc相关的一些内容，主要是帮助我们更好的理解lua的gc。
...
![数据流图](img/lua_stack_simple.png)

至于base/top和stack/stack_last，以及base_ci/end_ci之间的关系和区别就不打算详细介绍了。主要是lua的指令操作的实现也是在栈上实现的，以及函数调用的时候也要用到栈，只是他们在这个数组的不同区间。
...
# gc链表
这个链表是记录了lua里面所有的可回收对象，另外注意这是一个单向链表。真因为是单向链表，为了效率，才不会去整个遍历一遍，才会再增加扫描的链表等，这些后面详细介绍。
